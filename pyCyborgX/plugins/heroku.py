...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        cyborg = await eor(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            if variable in heroku_var:
                return await cyborg.eor(
        ...
        cyborg = await eor(var, "`Setting information...`")
        if not variable:
            return await cyborg.eor("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await cyborg.eor(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await cyborg.eor(
        ...
        cyborg = await eor(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await cyborg.eor("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await cyborg.eor(f"`{variable}`**  does not exist**")

        await cyborg.eor(f"`{variable}`  **successfully deleted**")
...
@Cyborg.on_cmd(
    ...
    info={
        "header": "To Check dyno usage of CyborgX and also to know how much left.",
        "usage": "{tr}usage",
    },
)
