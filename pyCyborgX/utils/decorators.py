    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                cyborgreg = "^" + Config.COMMAND_HAND_LER
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                cyborgreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/CyborgXUpdates)"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Cyborg.add_event_handler(func, MessageEdited(**args))
        Cyborg.add_event_handler(func, NewMessage(**args))
        try:
