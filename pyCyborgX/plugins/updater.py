...
from pyCyborgX import HEROKU_APP, UPSTREAM_REPO_URL, Cyborg
...
async def print_changelogs(event, ac_br, changelog):
    ...
    if len(changelog_str) > 4096:
        await event.eor("`Changelog is too big, view the file to see it.`")
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is None:
        return await event.eor("`Please set up`  **HEROKU_API_KEY**  ` Var...`")
    ...
    try:
        ...
        return repo.__del__()
    await event.eor("`Deploy was failed. So restarting to update`")
    delgvar("ipaddress")
    try:
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"do `{cmdhd}update deploy` to update the CyborgX")
    ...
    if conf == "now":
        await event.eor("`Updating userbot, please wait....`")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/Mahesh954-Hub/CyborgX"
    os.chdir("/app")
    try:
        txt = "`Oops.. Updater cannot continue due to "
        txt += "some problems occured`\n\n**LOGTRACE:**\n"
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.eor("`Deploying userbot, please wait....`")
