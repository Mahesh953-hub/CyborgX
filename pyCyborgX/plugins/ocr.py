...
from . import _cyborgtools, convert_toimage, deEmojify, Cyborg
...
async def ocr(event):
    ...
    cyborgevent = await eor(event, "`Reading...`")
    ...
    try:
        ...
        await cyborgevent.edit("`Couldn't read it.. you sure this readable !?`")
    test_file = await ocr_space_file(filename=output_file, language=lang_code)
    try:
        ParsedText = test_file["ParsedResults"][0]["ParsedText"]
    except BaseException:
        await cyborgevent.edit("`Couldn't read it.`\n`I guess I need new glasses.`")
    else:
        if cmd == "":
