...
async def cyborggban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    cyborgz = await eor(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cyborgz)
    if not user:
        return
    if user.id == Cyborg.uid:
        return await edit_delete(cyborgz, "`why would I ban myself`")
    if str(user.id) in DEVLIST:
        return await edit_delete(cyborgz, "🥴 **GBan my creator ?¿ Really‽**")
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.cyborggban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    madboy = len(san)
    if madboy == 0:
        return await edit_delete(cyborgz, "`you are not admin of atleast one group` ")
    await cyborgz.edit(
    ...
    cyborgtaken = (end - start).seconds
    if reason:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {cyborgtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {cyborgtaken} seconds`!!"
    ...
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**Time taken : **`{cyborgtaken} seconds`",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def cyborggban(event):
    "To unban the person from every group where you are admin."
    cyborgz = await eor(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cyborgz)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.cyborgungban(user.id)
    else:
        return await edit_delete(
            cyborgz, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = await admin_groups(event.client)
    count = 0
    madboy = len(san)
    if madboy == 0:
        return await edit_delete(
            cyborgz, "`you are not even admin of atleast one group `"
        )
    await cyborgz.edit(
    ...
    if reason:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {cyborgtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {cyborgtaken} seconds`!!"
...
async def cyborggkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    cyborgz = await eor(event, "`gkicking.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cyborgz)
    if not user:
        return
    if user.id == Cyborg.uid:
        return await edit_delete(cyborgz, "`why would I kick myself`")
    if str(user.id) in DEVLIST:
        return await edit_delete(cyborgz, "**😒 I'm not going to gkick my developer!!**")
    ...
    if reason:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {cyborgtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await cyborgz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {cyborgtaken} seconds`!!"
...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @CyborgXUpdates", str(err))
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
        await cyborg.edit("khud ko gpromote krega😑😑 chutiya spotted..")
        return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await cyborg.edit("**kuch to gadbad hai😑😑**")
    if user:
        telchanel = [
            d.entity.id
        ...
        for x in telchanel:
            try:
                ...
                i += 1
                await cyborg.edit(f"**Promoted in Chats **: `{i}`")
            except:
                pass
    else:
        await cyborg.edit("**kisi ko reply kr lavde🙄!!**")
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
        await cyborg.edit("khud ko gdemote karega😑😑 chumtiya spotted..")
        return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await cyborg.edit("**Phir se kuch gadbad kiya😑**")
    if user:
        telchanel = [
            d.entity.id
            for d in await userbot.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await userbot.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await cyborg.edit(f"**Demoted in Chats **: `{i}`")
            except:
                pass
    else:
        await cyborg.edit("**Reply to a user you dumbo !!**")
    return await cyborg.edit(
...
async def gcast(event):
    ...
    tt = event.text
    msg = tt[6:]
    cyborg = await eor(event, "`Globally Broadcasting Msg...`")
    er = 0
    done = 0
    async for x in bot.iter_dialogs():
        if x.is_group:
            ...
            try:
                ...
                er += 1
    await cyborg.edit(f"Done in {done} chats, error in {er} chat(s)")
