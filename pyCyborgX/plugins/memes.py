...
from ..helpers.utils import _cyborgutils, parse_pre
...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay CyborgX",
            "{tr}milksay CyborgX",
            "{tr}ghostbustersghostbusterssay CyborgX",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await cyborgmemes.slap(replied_user, event, mention)
    try:
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", " " + random.choice(cyborgmemes.UWUS), reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += " " + random.choice(cyborgmemes.UWUS)
...
async def gbun(event):
    ...
    cyborgevent = await eor(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await cyborgevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await cyborgevent.edit(mention)
