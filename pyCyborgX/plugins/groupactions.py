...
async def ban_user(chat_id, i, rights):
    try:
        await Cyborg(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def kickme(leave):
    "to leave the group."
    await leave.eor("`Nope, no, no, I go away`")
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if flag != " -r":
            ...
            cyborgadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not cyborgadmin:
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if status:
                    ...
                    await et.eor("I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
...
Bots: {}
None: {}"""
        await et.eor(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
        await sleep(5)
    await et.eor(
        """Total: {} users
Deleted Accounts: {}
