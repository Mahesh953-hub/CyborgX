...
async def tmuter(event):  # sourcery no-metrics
    ...
    cyborgevent = await eor(event, "`muting....`")
    user, reason = await get_user_from_event(event, cyborgevent)
    if not user:
        return
    if not reason:
        return await cyborgevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    cyborgtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(cyborgevent, cyborgtime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await cyborgevent.edit("Sorry, I can't mute myself")
    try:
        await cyborgevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
        ...
        return await cyborgevent.edit("`Uh oh my mute logic broke!`")
    except UserAdminInvalidError:
        return await cyborgevent.edit(
            "`Either you're not an admin or you tried to mute an admin that you didn't promote`"
        )
    except Exception as e:
        return await cyborgevent.edit(f"`{e}`")
...
async def tban(event):  # sourcery no-metrics
    ...
    cyborgevent = await eor(event, "`banning....`")
    user, reason = await get_user_from_event(event, cyborgevent)
    if not user:
        return
    if not reason:
        return await cyborgevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    cyborgtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await cyborgevent.edit("Sorry, I can't ban myself")
    await cyborgevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
        ...
        return await cyborgevent.edit(NO_PERM)
