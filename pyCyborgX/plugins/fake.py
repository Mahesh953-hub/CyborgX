...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, cyborgevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await cyborgevent.edit("__I think you don't have permission to promote__")
    except Exception as e:
        return await edit_delete(cyborgevent, f"__{e}__", time=10)
    await cyborgevent.edit("`Promoted Successfully! Now gib Party`")
