...
async def noods(event):
    ...
    cyborgevent = await eor(event, "🔍 `Searching...`")
    r = requests.get(
    ...
    if not walls:
        return await edit_delete(
            cyborgevent, f"**Can't find anything with** `{query}`", 10
        )
    i = count = 0
    piclist = []
    piclinks = []
    captionlist = []
    await eor(cyborgevent, "⏳ `Processing..`")
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            await eor(cyborgevent, "📥** Downloading...**")
            pic = await wall_download(a, query)
            if pic is None:
                return await edit_delete(
                    cyborgevent, "__Sorry i can't download wallpaper.__"
                )
            piclist.append(pic)
            piclinks.append(a)
            captionlist.append("")
            ...
            i += 1
        await eor(
            cyborgevent, f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**"
        )
        if count == int(limit):
            break
        if i == 5:
            await eor(cyborgevent, "`Max search error limit exceed..`")
    try:
        await eor(cyborgevent, "`Sending...`")
