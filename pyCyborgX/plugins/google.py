...
async def gsearch(q_event):
    ...
    cyborgevent = await eor(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        cyborgevent = await eor(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await cyborgevent.eor("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                cyborgevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        cyborgevent = event
    await eor(cyborgevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        cyborgevent = await eor(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await cyborgevent.eor("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await cyborgevent.eor("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            ...
            return await cyborgevent.eor("`Can't find any kind similar images.`")
