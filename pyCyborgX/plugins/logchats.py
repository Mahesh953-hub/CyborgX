...
@Cyborg.on_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
...
async def set_pmlog(event):
    ...
    if PMLOG:
        if h_type:
            await event.eor("`Pm logging is already enabled`")
        else:
            addgvar("PMLOG", h_type)
            await event.eor("`Pm logging is disabled`")
    elif h_type:
        addgvar("PMLOG", h_type)
        await event.eor("`Pm logging is enabled`")
    else:
        await event.eor("`Pm logging is already disabled`")
...
async def set_grplog(event):
    ...
    if GRPLOG:
        if h_type:
            await event.eor("`Group logging is already enabled`")
        else:
            addgvar("GRPLOG", h_type)
            await event.eor("`Group logging is disabled`")
    elif h_type:
        addgvar("GRPLOG", h_type)
        await event.eor("`Group logging is enabled`")
    else:
        await event.eor("`Group logging is already disabled`")
