...
You have {warns}/{totalwarns} warns until you get blocked by the CyborgX
...
Don't spam my inbox. say reason and wait until my response.__"""
    ...
    try:
        if gvarstatus("pmmenu") is None:
            ...
            if PM_PIC:
                CYBORG = list(PM_PIC.split())
                PIC = list(CYBORG)
                CYBORG_IMG = random.choice(PIC)
            else:
                CYBORG_IMG = None
            if CYBORG_IMG is not None:
...
@Cyborg.on_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@Cyborg.on_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
**Choose one of the following reasons why you are here:**"""
    ...
    if str(event.query.user_id) in PM_WARNS:
        ...
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
    await event.eor(text, buttons=buttons)


@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
