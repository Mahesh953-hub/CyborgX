...
async def icalc(e):
    if e.client._bot:
        return await e.reply("**𝙄𝙉𝙇𝙄𝙉𝙀 𝘾𝙔𝘽𝙊𝙍𝙂𝙓 𝘾𝘼𝙇𝘾𝙐𝙇𝘼𝙏𝙊𝙍**", buttons=lst)
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**𝙄𝙉𝙇𝙄𝙉𝙀 𝘾𝙔𝘽𝙊𝙍𝙂𝙓 𝘾𝘼𝙇𝘾𝙐𝙇𝘼𝙏𝙊𝙍**", buttons=lst
        )
        await event.answer([calc])


@Cyborg.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**𝐈𝐧𝐥𝐢𝐧𝐞 𝐂𝐲𝐛𝐨𝐫𝐠𝐗 𝐂𝐚𝐥𝐜𝐮𝐥𝐚𝐭𝐞𝐫**", buttons=lst)
