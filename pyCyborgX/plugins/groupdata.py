...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @CyborgXSupport",
    },
)
async def _(event):
    "To get list of bots."
...
async def get_users(show):
    ...
    cyborgevent = await eor(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await eor(cyborgevent, mentions)
...
async def info(event):
    ...
    cyborgevent = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, cyborgevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await cyborgevent.eor(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await cyborgevent.eor("`An unexpected error has occurred.`")


async def get_chatinfo(event, cyborgevent):
    ...
    try:
        ...
        try:
            ...
            await cyborgevent.eor("`Invalid channel/group`")
            return None
            ...
            await cyborgevent.eor("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await edit_delete(cyborgevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
