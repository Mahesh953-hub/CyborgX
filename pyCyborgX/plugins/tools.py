...
from ..helpers.utils import _format, _cyborgutils, reply_id
...
async def _(event):
    ...
    chat = "@VS_Robot"
    cyborgevent = await eor(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await eor(cyborgevent, response4.text)
...
async def parseqr(event):
    ...
    cyborgevent = await eor(event, "`Decoding....`")
    ...
    t_response, e_response = (await _cyborgutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await eor(cyborgevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await eor(cyborgevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await eor(cyborgevent, f"**Error:**\n`{e}`")
...
async def _(event):
    ...
    try:
        ...
        return await cyborgevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(cyborgevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    if not r.ok:
        return await cyborgevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
