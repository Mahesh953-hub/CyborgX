...
async def _(event):
    ...
    cyborgevent = await eor(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await cyborgevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await cyborgevent.edit(r["error"])
    ...
    await cyborgevent.edit(string_to_show)
