...
async def grpinfo():
    outstr = "**Plugins in CyborgX are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for cyborg in category:
        plugins = GRP_INFO[cyborg]
        outstr += f"**{hemojis[cyborg]} {cyborg.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your CyborgX are :**\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for cyborg in category:
        plugins = GRP_INFO[cyborg]
        outstr += f"**{hemojis[cyborg]} {cyborg.title()} ** - {len(plugins)}\n\n"
...
@Cyborg.on_cmd(
    ...
    info={
        "header": "To get guide for CyborgX.",
...
async def _(event):
    ...
    await eor(
        event, outstr, aslink=True, linktext="Total Commands of CyborgX are :"
    )
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in CyborgX"
    await eor(event, out)
