...
async def promote(event):
    ...
    if not user:
        return
    cyborgevent = await eor(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await cyborgevent.eor(NO_PERM)
    await cyborgevent.eor("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    cyborgevent = await eor(event, "`Demoting...`")
    ...
    try:
        ...
        return await cyborgevent.eor(NO_PERM)
    await cyborgevent.eor("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    cyborgevent = await eor(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    cyborgevent = await eor(event, "`Unbanning...`")
    try:
        ...
        await cyborgevent.eor("`Uh oh my unban logic broke!`")
    except Exception as e:
...
async def startmute(event):
    ...
    if event.is_private:
        await event.eor("`Unexpected issues or ugly errors may occur!`")
        await sleep(2)
        ...
        try:
            ...
            await event.eor("`Successfully muted that person.\n**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "#PM_MUTE\n"
...
async def endmute(event):
    ...
    if not user:
        return
    cyborgevent = await eor(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await cyborgevent.eor(f"{NO_PERM}\n{e}")
    if reason:
        await cyborgevent.eor(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await cyborgevent.eor(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    cyborgevent = await eor(event, "`Searching recent actions .....`")
    ...
    if not flag:
        ...
        await eor(cyborgevent, deleted_msg)
    else:
        main_msg = await eor(cyborgevent, deleted_msg)
        for msg in adminlog:
            ruser = (
                await event.client(GetFullUserRequest(msg.old.from_id.user_id))
            ).user
