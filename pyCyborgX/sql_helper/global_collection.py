...
class Cyborg_GlobalCollection(BASE):
    __tablename__ = "cyborg_globalcollection"
    ...
    def __repr__(self):
        return "<CyborgX Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Cyborg_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Cyborg_GlobalCollection.__table__.create(checkfirst=True)

CYBORG_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with CYBORG_GLOBALCOLLECTION:
        keyword_items = Cyborg_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with CYBORG_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Cyborg_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with CYBORG_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Cyborg_GlobalCollection.keywoard)
            .filter(Cyborg_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Cyborg_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Cyborg_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Cyborg_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Cyborg_GlobalCollection).all()
        for x in all_groups:
