...
async def paste_img(event):
    ...
    d_file_name = None
    cyborgevent = await eor(event, "`Pasting the text on image`")
    ...
    try:
        ...
        await edit_delete(cyborgevent, f"**Error:**\n`{e}`", time=10)
...
async def paste_bin(event):
    ...
    cyborgevent = await eor(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await cyborgevent.eor(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(cyborgevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    cyborgevent = await eor(event, "`Getting Contents of pastebin.....`")
    rawurl = None
    if "raw" in url:
        rawurl = url
    if rawurl is None:
    ...
    try:
        ...
        return await cyborgevent.eor(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await eor(cyborgevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(cyborgevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await cyborgevent.eor("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await cyborgevent.eor("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await cyborgevent.eor(result, link_preview=True)
