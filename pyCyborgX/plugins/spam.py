...
from ..helpers.utils import _cyborgutils
...
async def spam_function(event, nadan, cyborg, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(cyborg[0])
    if len(cyborg) == 2:
        spam_message = str(cyborg[1])
...
async def spammer(event):
    ...
    await spam_function(event, nadan, cyborg, sleeptimem, sleeptimet)
...
async def stickerpack_spam(event):
    ...
    try:
        ...
        cyborgevent = await edit_or_reply(
...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using CyborgX",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, cyborg, sleeptimem, sleeptimet, DelaySpam=True)
