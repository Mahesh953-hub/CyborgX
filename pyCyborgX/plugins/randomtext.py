...
async def _(event):
    ...
    if x == 33:
        await event.eor("Not in a mood to sing. Sorry!")
...
async def _(event):  # sourcery no-metrics
    ...
    if x == 1:
        await event.eor("**Aberto**")
    if x == 2:
        await event.eor("**Accio**")
    if x == 3:
        await event.eor("**Aguamenti**")
    if x == 4:
        await event.eor("**Alohomora**")
    if x == 5:
        await event.eor("**Avada Kedavra**")
    if x == 6:
        await event.eor("**Colloportus**")
    if x == 7:
        await event.eor("**Confringo**")
    if x == 8:
        await event.eor("**Confundo**")
    if x == 9:
        await event.eor("**Crucio**")
    if x == 10:
        await event.eor("**Descendo**")
    if x == 11:
        await event.eor("**Diffindo**")
    if x == 12:
        await event.eor("**Engorgio**")
    if x == 13:
        await event.eor("**Episkey**")
    if x == 14:
        await event.eor("**Evanesco**")
    if x == 15:
        await event.eor("**Expecto Patronum**")
    if x == 16:
        await event.eor("**Expelliarmus**")
    if x == 17:
        await event.eor("**Finestra**")
    if x == 18:
        await event.eor("**Homenum Revelio**")
    if x == 19:
        await event.eor("**Impedimenta**")
    if x == 20:
        await event.eor("**Imperio**")
    if x == 21:
        await event.eor("**Impervius**")
    if x == 22:
        await event.eor("**Incendio**")
    if x == 23:
        await event.eor("**Levicorpus**")
    if x == 24:
        await event.eor("**Lumos**")
    if x == 25:
        await event.eor("**Muffliato**")
    if x == 26:
        await event.eor("**Obliviate**")
    if x == 27:
        await event.eor("**Petrificus Totalus**")
    if x == 28:
        await event.eor("**Priori Incantato**")
    if x == 29:
        await event.eor("**Protego**")
    if x == 30:
        await event.eor("**Reducto**")
    if x == 31:
        await event.eor("**Rennervate**")
    if x == 32:
        await event.eor("**Revelio**")
    if x == 33:
        await event.eor("**Rictusempra**")
    if x == 34:
        await event.eor("**Riddikulus**")
    if x == 35:
        await event.eor("**Scourgify**")
    if x == 36:
        await event.eor("**Sectumsempra**")
    if x == 37:
        await event.eor("**Silencio**")
    if x == 37:
        await event.eor("**Stupefy**")
    if x == 38:
        await event.eor("**Tergeo**")
    if x == 39:
        await event.eor("**Wingardium Leviosa**")
...
async def _(event):  # sourcery no-metrics
    ...
    x = random.randrange(1, 40)
    if x == 1:
        await event.eor('`"The man who passes the sentence should swing the sword."`')
    if x == 2:
        await event.eor(
            '`"When the snows fall and the white winds blow, the lone wolf dies but the pack survives!"`'
        )
    if x == 3:
        await event.eor('`"The things I do for love!"`')
    if x == 4:
        await event.eor(
    ...
    if x == 7:
        await event.eor('`"If I look back I am lost."`')
    if x == 8:
        await event.eor('`"When you play the game of thrones, you win or you die."`')
    if x == 9:
        await event.eor(
            '`"I grew up with soldiers. I learned how to die a long time ago."`'
        )
    if x == 10:
        await event.eor('`"What do we say to the Lord of Death?\nNot Today!"`')
    if x == 11:
        await event.eor('`"Every flight begins with a fall."`')
    if x == 12:
        await event.eor('`"Different roads sometimes lead to the same castle."`')
    if x == 13:
        await event.eor(
    ...
    if x == 15:
        await event.eor('`"The night is dark and full of terrors."`')
    if x == 16:
        await event.eor('`"You know nothing, Jon Snow."`')
    if x == 17:
        await event.eor('`"Night gathers, and now my watch begins!"`')
    if x == 18:
        await event.eor('`"A Lannister always pays his debts."`')
    if x == 19:
        await event.eor('`"Burn them all!"`')
    if x == 20:
        await event.eor('`"What do we say to the God of death?"`')
    if x == 21:
        await event.eor('`"There\'s no cure for being a c*nt."`')
    if x == 22:
        await event.eor('`"Winter is coming!"`')
    if x == 23:
        await event.eor('`"That\'s what I do: I drink and I know things."`')
    if x == 24:
        await event.eor(
    ...
    if x == 26:
        await event.eor('`"Chaos isn\'t a pit. Chaos is a ladder."`')
    if x == 27:
        await event.eor(
    ...
    if x == 30:
        await event.eor('`"A girl is Arya Stark of Winterfell. And I\'m going home."`')
    if x == 31:
        await event.eor("`\"Any man who must say 'I am the King' is no true King.\"`")
    if x == 32:
        await event.eor('`"If I fall, don\'t bring me back."`')
    if x == 33:
        await event.eor(
            "`\"Lannister, Targaryen, Baratheon, Stark, Tyrell... they're all just spokes on a wheel. This one's on top, then that one's on top, and on and on it spins, crushing those on the ground.\"`"
        )
    if x == 34:
        await event.eor('`"Hold the door!`')
    if x == 35:
        await event.eor(
            '`"When people ask you what happened here, tell them the North remembers. Tell them winter came for House Frey."`'
        )
    if x == 36:
        await event.eor('`"Nothing f*cks you harder than time."`')
    if x == 37:
        await event.eor(
            '`"There is only one war that matters. The Great War. And it is here."`'
        )
    if x == 38:
        await event.eor('`"Power is power!"`')
    if x == 39:
        await event.eor('`"I demand a trial by combat!"`')
    if x == 40:
        await event.eor('`"I wish I was the monster you think I am!"`')
    if x == 41:
        await event.eor(
            "Never forget what you are. The rest of the world will not.Wear it like armor,\n and it can never be used to hurt you."
        )
    if x == 42:
        await event.eor("There is only one thing we say to death: **Not today.**")
    if x == 43:
        await event.eor(
            "If you think this has a happy ending, you haven’t been **paying attention**."
        )
...
async def _(event):  # sourcery no-metrics
    ...
    if x == 4:
        await event.eor('`"What scene from a TV show will you never forget?"`')
    if x == 5:
        await event.eor(
            '`"If you could become a master in one skill, what skill would you choose?"`'
        )
    if x == 6:
        await event.eor('`"What three words can describe you?"`')
    if x == 7:
        await event.eor(
    ...
    if x == 9:
        await event.eor('`"If you switched genders for the day, what would you do?"`')
    if x == 10:
        await event.eor(
    ...
    if x == 14:
        await event.eor('`"How many kids do you want to have in the future?"`')
    if x == 15:
        await event.eor(
            '`"Who in this group would be the worst person to date? Why?"`'
        )
    if x == 16:
        await event.eor('`"What does your dream boy or girl look like?"`')
    if x == 17:
        await event.eor(
            '`"What would be in your web history that you’d be embarrassed if someone saw?"`'
        )
    if x == 18:
        await event.eor('`"Do you sing in the shower?"`')
    if x == 19:
        await event.eor('`"What’s the right age to get married?"`')
    if x == 20:
        await event.eor('`"What are your top 5 rules for life?"`')
    if x == 21:
        await event.eor(
    ...
    if x == 26:
        await event.eor('`"Live-in relation or marriage, what do you prefer?"`')
    if x == 27:
        await event.eor('`"What is your favorite cuisine or type of food?"`')
    if x == 28:
        await event.eor(
            '`"What are some good and bad things about the education system in your country?"`'
        )
    if x == 29:
        await event.eor('`"What do you think of online education?"`')
    if x == 30:
        await event.eor('`"What are some goals you have failed to accomplish?"`')
    if x == 31:
        await event.eor('`"Will technology save the human race or destroy it?"`')
    if x == 32:
        await event.eor('`"What was the best invention of the last 50 years?"`')
    if x == 33:
        await event.eor(
    ...
    if x == 35:
        await event.eor('`"What’s the most addictive mobile game you have played?"`')
    if x == 36:
        await event.eor('`"How many apps do you have on your phone?"`')
    if x == 37:
        await event.eor('`"What was the last song you listened to?"`')
    if x == 38:
        await event.eor(
            '`"Do you prefer to watch movies in the theater or in the comfort of your own home?"`'
        )
    if x == 39:
        await event.eor('`"Do you like horror movies? Why or why not?"`')
    if x == 40:
        await event.eor(
            '`"How often do you help others? Who do you help? How do you help?"`'
        )
    if x == 41:
        await event.eor('`"What song do you play most often?"`')
    if x == 42:
        await event.eor('`"Suggest a new rule that should be added in this group!"`')
    if x == 43:
        await event.eor('`"What app on your phone do you think I should get?"`')
    if x == 44:
        await event.eor(
            '`"What website or app has completely changed your life for better or for worse?"`'
        )
    if x == 45:
        await event.eor('`"What isn’t real but you desperately wish it was?"`')
    if x == 46:
        await event.eor('`"What thing do you really wish you could buy right now?"`')
    if x == 47:
        await event.eor(
    ...
    if x == 49:
        await event.eor('`"Who is the luckiest person you know?"`')
    if x == 50:
        await event.eor(
            '`"If you could visit someone\'s house in this group, who would it be ?"`'
        )
    if x == 51:
        await event.eor('`"What are you tired of hearing about?"`')
    if x == 52:
        await event.eor(
            '`"If you died today, what would your greatest achievement be?"`'
        )
    if x == 53:
        await event.eor('`"What method will you choose to kill yourself?"`')
    if x == 54:
        await event.eor('`"What’s the best news you\'ve heard in the last 24 hours?"`')
    if x == 55:
        await event.eor(
            '`"What is the most important change that should be made to your country’s education system?"`'
        )
    if x == 56:
        await event.eor('`"Send your favourite sticker pack."`')
    if x == 57:
        await event.eor('`"Send your favourite animated sticker pack."`')
    if x == 58:
        await event.eor('`"Send your favourite video or gif."`')
    if x == 59:
        await event.eor('`"Send your favourite emojies"`')
    if x == 60:
        await event.eor(
...
async def _(event):  # sourcery no-metrics
    ...
    if x == 99:
        await event.eor("`Do You Know, Mosquitoes has Teleportation Power...`")
    if x == 100:
        await event.eor(
    ...
    if x == 102:
        await event.eor("`Do You Know, Bus are called Bus, Because they are Bus....`")
    if x == 103:
        await event.eor(
            "`Do You Know, There's a Huge Difference between *Cartoon amd Anime*...`"
        )
    if x == 104:
        await event.eor("`Do You Know, We can't see Ghosts But Ghosts Can see Us...`")
