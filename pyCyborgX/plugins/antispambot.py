...
from . import BOTLOG, BOTLOG_CHATID, eor, Cyborg, logging, spamwatch

LOGS = logging.getLogger(__name__)
plugin_category = "admin"
if Config.ANTISPAMBOT_BAN:

    @Cyborg.on(ChatAction())
    async def anti_spambot(event):
        if not event.user_joined and not event.user_added:
            return
        user = await event.get_user()
        cyborgadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not cyborgadmin:
            return
        cyborgbanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            cyborggban = get_gbanuser(user.id)
            if cyborggban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{cyborggban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not cyborgbanned:
...
async def caschecker(event):
    ...
    try:
        ...
        return await event.eor(str(err))
    try:
        ...
        await cyborgevent.eor("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await cyborgevent.eor("`CAS check failed`")
        return
    await cyborgevent.eor(text)
...
async def caschecker(event):
    ...
    try:
        ...
        await cyborgevent.eor("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await cyborgevent.eor("`spamwatch check failed`")
        return
    await cyborgevent.eor(text)
