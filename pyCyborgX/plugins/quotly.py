...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic CyborgX.", "{tr}qpic -b CyborgX."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    cyborgevent = await eor(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "CyborgX.Webp"
        img.save(output, "webp")
    else:
        output.name = "CyborgX.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def stickerchat(cyborgquotes):
    "Makes your message as sticker quote"
    reply = await cyborgquotes.get_reply_message()
    if not reply:
        return await eor(
            cyborgquotes, "`I cant quote the message . reply to a message`"
        )
    fetchmsg = reply.message
    repliedreply = None
    mediatype = media_type(reply)
    if mediatype and mediatype in ["Photo", "Round Video", "Gif"]:
        return await eor(cyborgquotes, "`Replied message is not supported now`")
    cyborgevent = await eor(cyborgquotes, "`Making quote...`")
    user = (
        await cyborgquotes.client.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, cyborgmsg = await process(fetchmsg, user, cyborgquotes.client, reply, repliedreply)
    if not res:
        return
    outfi = os.path.join("./temp", "sticker.png")
    cyborgmsg.save(outfi)
    endfi = convert_tosticker(outfi)
    await cyborgquotes.client.send_file(cyborgquotes.chat_id, endfi, reply_to=reply)
    await cyborgevent.delete()
    os.remove(endfi)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    cyborgevent = await eor(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    cyborgevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await cyborgevent.eor("```Please unblock me (@QuotLyBot)```")
