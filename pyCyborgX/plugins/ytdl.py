...
async def ytdl_down(event, opts, url):
    ytdl_data = None
    try:
        await event.eor("`Fetching data, please wait..`")
        with YoutubeDL(opts) as ytdl:
            ytdl_data = ytdl.extract_info(url)
    except DownloadError as DE:
        await event.eor(f"`{DE}`")
    except ContentTooShortError:
        await event.eor("`The download content was too short.`")
    except GeoRestrictedError:
        await event.eor(
            "`Video is not available from your geographic location due to geographic restrictions imposed by a website.`"
        )
    except MaxDownloadsReached:
        await event.eor("`Max-downloads limit has been reached.`")
    except PostProcessingError:
        await event.eor("`There was an error during post processing.`")
    except UnavailableVideoError:
        await event.eor("`Media is not available in the requested format.`")
    except XAttrMetadataError as XAME:
        await event.eor(f"`{XAME.code}: {XAME.msg}\n{XAME.reason}`")
    except ExtractorError:
        await event.eor("`There was an error during info extraction.`")
...
async def download_audio(event):
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        return await edit_delete(cyborgevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    cyborgevent = await eor(event, "`Preparing to download...`")
    reply_to_id = await reply_id(event)
    ytdl_data = await ytdl_down(cyborgevent, video_opts, url)
    if ytdl_down is None:
        return
    f = pathlib.Path(f"{ytdl_data['title']}.mp4".replace("|", "_"))
    cyborgthumb = pathlib.Path(f"{ytdl_data['title']}.jpg".replace("|", "_"))
    if not os.path.exists(cyborgthumb):
        cyborgthumb = pathlib.Path(f"{ytdl_data['title']}.webp".replace("|", "_"))
    if not os.path.exists(cyborgthumb):
        cyborgthumb = None
    await cyborgevent.eor(
        f"`Preparing to upload video:`\
    ...
    uploaded = await event.client.fast_upload_file(
        file=ul,
        progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
            progress(d, t, cyborgevent, c_time, "upload", file_name=f)
        ),
    )
    ul.close()
    media = types.InputMediaUploadedDocument(
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        cyborgevent = await eor(event, "**Downloading.....**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await cyborgevent.eor("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await cyborgevent.delete()
        cyborg = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()
