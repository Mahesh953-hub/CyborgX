...
from . import edit_delete, Cyborg, reply_id
...
async def _(event):
    ...
    if not markdown_note:
        return await edit_delete(event, "`what text should i use in button post`")
    cyborginput = f"Inline buttons {markdown_note}"
    results = await event.client.inline_query(Config.TG_BOT_USERNAME, cyborginput)
