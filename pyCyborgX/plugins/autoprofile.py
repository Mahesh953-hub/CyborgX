...
autopic_path = os.path.join(os.getcwd(), "pyCyborgX", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "pyCyborgX", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "pyCyborgX", "photo_pfp.png")
...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await Cyborg.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await Cyborg.upload_file(autophoto_path)
        try:
            await Cyborg(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await Cyborg.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await Cyborg(
                    functions.photos.DeletePhotosRequest(
                        await Cyborg.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await Cyborg(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        drawn_text = ImageDraw.Draw(img)
        cyborg = str(
            base64.b64decode("cHlDeWJvcmdYL2hlbHBlcnMvc3R5bGVzL2RpZ2l0YWwudHRm")
        )[2:36]
        fnt = ImageFont.truetype(cyborg, 200)
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await Cyborg(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await Cyborg(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await Cyborg(functions.photos.UploadProfilePhotoRequest(file))
        await _cyborgutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def _(event):
    ...
    await event.eor("`Starting batman Profile Pic.`")
    await autopfp_start()
...
async def _(event):
    ...
    await event.eor("`Starting thor Profile Pic.`")
    await autopfp_start()
...
Cyborg.loop.create_task(autopfp_start())
Cyborg.loop.create_task(autopicloop())
Cyborg.loop.create_task(digitalpicloop())
Cyborg.loop.create_task(bloom_pfploop())
Cyborg.loop.create_task(autoname_loop())
Cyborg.loop.create_task(autobio_loop())
Cyborg.loop.create_task(custompfploop())
