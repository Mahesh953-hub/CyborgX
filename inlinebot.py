...
CYBORGLOGO = "https://telegra.ph/file/ddc5fa84192641f0915e3.jpg"
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**CyborgX"):
            ...
            if IALIVE_PIC:
                CYBORG = [x for x in IALIVE_PIC.split()]
                PIC = list(CYBORG)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                CYBORG = [x for x in ALIVE_PIC.split()]
                PIC = list(CYBORG)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            builder = event.builder
            troll = os.path.join("./pyCyborgX", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./pyCyborgX", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./pyCyborgX", "hide.txt")
            try:
            ...
            result = builder.article(
                title="© CyborgX Help",
                description="Help Menu For CyborgX",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if PM_PIC:
                CYBORG = [x for x in PM_PIC.split()]
                PIC = list(CYBORG)
                CYBORGIMG = random.choice(PIC)
            else:
                CYBORGIMG = None
            query = gvarstatus("pmpermit_text")
            if CYBORGIMG and CYBORGIMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    CYBORGIMG,
                    # title="ᴀʟɪᴠᴇ ᴄʏʙᴏʀɢx",
                    text=query,
        ...
        buttons = [
            (
                ...
                Button.url("Support", "https://t.me/CyborgXUpdates"),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        photo = types.InputWebDocument(
            url=CYBORGLOGO, size=0, mime_type="image/jpeg", attributes=[]
        )
        text, msg_entities = await event.client._parse_message_text(
            "𝐃𝐞𝐩𝐥𝐨𝐲 𝐘𝐨𝐮𝐫 𝐎𝐰𝐧 𝐂𝐲𝐛𝐨𝐫𝐠𝐗 .", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Cyborg.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
