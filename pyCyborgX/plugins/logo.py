...
async def bad(event):
    ...
    for i, each in enumerate(links, start=1):
        cyborg = os.path.splitext(each.text)[0]
        bg_name.append(cyborg)
        lbg_list += f"**{i}.**  `{cyborg}`\n"
    ...
    if input_str not in bg_name:
        cyborgevent = await eor(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(cyborgevent, lbg_list, time=60)
    else:
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        for i, each in enumerate(links, start=1):
            cyborg = os.path.splitext(each.text)[0]
            logo_font.append(cyborg)
            font_name += f"**{i}.**  `{cyborg}`\n"
        if not input_str:
        ...
        if input_str not in logo_font:
            ...
            await asyncio.sleep(1)
            await edit_delete(cyborgevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            cyborgevent = await eor(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                cyborgevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        cyborg = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(cyborg, input_str)
