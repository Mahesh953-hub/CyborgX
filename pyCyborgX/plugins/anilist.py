...
async def anilist_usersearch(event):
    ...
    cyborgevent = await eor(event, "`Searching user profile in anilist...`")
    searchresult = await anilist_user(search_query)
    if len(searchresult) == 1:
        return await eor(
            cyborgevent, f"**Error while searching user profile:**\n{searchresult[0]}"
        )
...
async def anilist(event):
    ...
    await event.eor(msg, link_preview=True)
...
async def get_manga(event):
    ...
    cyborgevent = await eor(event, "`Searching Manga..`")
...
async def get_anime(event):
    ...
    cyborgevent = await eor(event, "`Searching Anime..`")
    jikan = jikanpy.jikan.Jikan()
...
async def character(event):
    ...
    cyborgevent = await eor(event, "`Searching Character...`")
    try:
        search_result = jikan.search("character", search_query)
    except APIException:
        return await edit_delete(cyborgevent, "`Character not found.`")
    first_mal_id = search_result["results"][0]["mal_id"]
...
async def anime_download(event):  # sourcery no-metrics
    ...
    if input_str == "kaizoku":
        ...
        if search_result:
            ...
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>IndiAnime</code>"
    await cyborgevent.eor(result, parse_mode="html")
...
async def whatanime(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"https://api.trace.moe/search?anilistInfo&url={quote_plus(cyborg)}"
        ) as raw_resp0:
