...
async def echo(event):
    ...
    cyborgevent = await eor(event, "`Adding Echo to user...`")
    user, rank = await get_user_from_event(event, cyborgevent, nogroup=True)
    if not user:
        return
...
@Cyborg.on_cmd(incoming=True, edited=False)
