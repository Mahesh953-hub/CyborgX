...
async def _(event):
    ...
    cyborgevent = await eor(event, "`wi8..! I am finding your song....`")
    ...
    try:
        ...
        pass
    stderr = (await _cyborgutils.runcmd(song_cmd))[1]
    if stderr:
        return await cyborgevent.eor(f"**Error :** `{stderr}`")
    cyborgname, stderr = (await _cyborgutils.runcmd(name_cmd))[:2]
    if stderr:
        return await cyborgevent.eor(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    cyborgname = os.path.splitext(cyborgname)[0]
    # if stderr:
    #    return await cyborgevent.eor(f"**Error :** `{stderr}`")
    song_file = Path(f"{cyborgname}.mp3")
    if not os.path.exists(song_file):
        return await cyborgevent.eor(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await cyborgevent.eor("`yeah..! i found something wi8..🥰`")
    cyborgthumb = Path(f"{cyborgname}.jpg")
    if not os.path.exists(cyborgthumb):
        cyborgthumb = Path(f"{cyborgname}.webp")
    elif not os.path.exists(cyborgthumb):
        cyborgthumb = None
    ...
    for files in (cyborgthumb, song_file):
...
async def _(event):
    ...
    stderr = (await _cyborgutils.runcmd(video_cmd))[1]
    if stderr:
        return await cyborgevent.eor(f"**Error :** `{stderr}`")
    cyborgname, stderr = (await _cyborgutils.runcmd(name_cmd))[:2]
    if stderr:
    ...
    #    return await cyborgevent.eor(f"**Error :** `{stderr}`")
    cyborgname = os.path.splitext(cyborgname)[0]
    vsong_file = Path(f"{cyborgname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{cyborgname}.mkv")
    ...
    await cyborgevent.delete()
    for files in (cyborgthumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    cyborgevent = await eor(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            cyborgevent, f"**Error while reverse searching song:**\n__{e}__"
        )
...
async def _(event):
    ...
    cyborgevent = await eor(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    cyborgevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await cyborgevent.eor(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await cyborgevent.eor(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    cyborgevent = await eor(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await cyborgevent.eor("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await cyborgevent.eor("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await cyborgevent.eor(namem)
