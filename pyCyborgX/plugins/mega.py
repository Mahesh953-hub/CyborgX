...
async def mega_downloader(megadl):  # sourcery no-metrics
    ...
    cyborgevent = await edit_or_reply(megadl, "`Collecting information...`")
    ...
    if link:
        ...
        return await cyborgevent.eor("Usage: `.mega` **<MEGA.nz link>**")
    try:
        ...
        if "file" in link:
            ...
            await cyborgevent.eor("`folder download support are removed...`")
            return
    except IndexError:
        await cyborgevent.eor("`MEGA.nz link not found...`")
        return None
    cmd = f"bin/megadown -q -m {link}"
    result = await subprocess_run(cyborgevent, cmd)
    try:
        data = json.loads(result[0])
    except json.JSONDecodeError:
        await cyborgevent.eor("**JSONDecodeError**: `failed to extract link...`")
        return None
    ...
    try:
        ...
        await cyborgevent.eor(f"**HTTPError**: `{str(e)}`")
        return None
    start = time.time()
    while not downloader.isFinished():
        ...
        try:
            ...
            if round(diff % 15.00) == 0 and (
                ...
                await cyborgevent.eor(current_message)
        ...
        try:
            P = multiprocessing.Process(
                target=await decrypt_file(
                    cyborgevent, file_path, temp_file_path, hex_key, hex_raw_key
                ),
                name="Decrypt_File",
            )
            P.start()
...
async def decrypt_file(megadl, file_path, temp_file_path, hex_key, hex_raw_key):
    cmd = "cyborg '{}' | openssl enc -d -aes-128-ctr -K {} -iv {} > '{}'".format(
