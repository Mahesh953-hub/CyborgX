...
async def _(event):
    ...
    if not replied_user:
        return
    cyborgevent = await eor(event, "`Fetching userinfo wait....`")
...
async def who(event):
    ...
    if not replied_user:
        return
    cyborg = await eor(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await eor(cyborg, "`Could not fetch info of that user.`")
    ...
    try:
        ...
        await cyborg.edit(caption, parse_mode="html")
