...
def verify_cond(cyborgarray, text):
    return any(i in text for i in cyborgarray)


def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"CyborgX_{userid}_{pack}_anim"
    if is_video:
        return f"CyborgX_{userid}_{pack}_vid"
    return f"CyborgX_{userid}_{pack}"


def char_is_emoji(character):
    return character in cyborgemoji.UNICODE_EMOJI["en"]
...
async def delpack(cyborgevent, conv, cmd, args, packname):
    try:
        await conv.send_message(cmd)
    except YouBlockedUserError:
        await cyborgevent.eor("You have blocked the @stickers bot. unblock it and try.")
        return None, None
...
async def add_to_pack(
    ...
    while ("50" in x.text) or ("120" in x.text):
        ...
        await cyborgevent.eor(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"cyborg_{user.id}"
    else:
        username = user.username
    userid = user.id
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            cyborgevent = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                cyborgevent = await eor(args, "__⌛ Downloading..__")
                sticker = await animator(message, args, cyborgevent)
                await eor(cyborgevent, f"`{random.choice(KANGING_STR)}`")
            is_video = True
            emoji = "😂"
            emojibypass = True
            photo = 1
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await cyborgevent.eor("check `.info stickers`")
        ...
        if (
            ...
            await cyborgevent.eor("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    cyborg = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(cyborgevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, cyborgXname = await newpacksticker(
                        cyborgevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, cyborgXname = await add_to_pack(
                        cyborgevent,
                        conv,
                        event,
                        packname,
            ...
            if cyborgXname is None:
                return
            if cyborgXname not in blablapacks:
                blablapacks.append(cyborgXname)
...
async def CyborgRoar(args):
    "To kang a sticker."  # scam :('  Dom't kamg :/@Teamcyborgx
    message = await args.get_reply_message()
    user = await args.client.get_me()
    if not user.username:
        try:
    ...
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"CyborgX_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
    ...
    await cyborgevent.eor("`Hold on, making sticker...`")
    async with args.client.conversation("@Stickers") as conv:
        otherpack, packname, emoji = await newpacksticker(
            cyborgevent,
            conv,
...
async def pic2packcmd(event):
    ...
    cyborgevent = await eor(event, "__🔪Cropping and adjusting the image...__")
    try:
    ...
    image = await _cyborgtools.media_to_pic(cyborgevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    cyborgevent = await event.eor("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await cyborgevent.eor("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    cyborgevent = await eor(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(cyborgevent, "`No results found :(.`", 5)
