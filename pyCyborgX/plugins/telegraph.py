...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        await cyborgevent.edit(f"`Downloaded to {downloaded_file_name}`")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await cyborgevent.edit(f"**Error : **\n`{exc}`")
        ...
        ms = (end - start).seconds
        cyborg = f"https://telegra.ph/{response['path']}"
        await cyborgevent.edit(
            f"**➥ Uploaded to :-** [telegraph]({cyborg})\
                 \n**➥ Uploaded in {ms} seconds.**\
                 \n**➥ Uploaded by :-** {mention}",
            link_preview=True,
        )
