...
async def animator(media, mainevent, textevent):
    ...
    CyborgX = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {CyborgX} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    ) 
    os.remove(CyborgX)
    return "animate.webm"
...
async def clippy(cyborg, msg, chat_id, reply_to_id):
    chat = "@clippy"
    async with cyborg.conversation(chat) as conv:
        try:
            msg = await conv.send_file(msg)
            pic = await conv.get_response()
            await cyborg.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await kakashi.edit("Please unblock @clippy and try again")
            return
        await cyborg.send_file(
            chat_id,
            pic,
            reply_to=reply_to_id,
        )
    await cyborg.delete_messages(conv.chat_id, [msg.id, pic.id])
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    Cyborg = await sticcers[c_lick].click("me", hide_via=True)
    if Cyborg:
        await borg.send_file(int(chat_id), Cyborg, reply_to=reply_to_id)
        await borg.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    cyborg = await sticcers[0].click("me", hide_via=True)
    if cyborg:
        await bot.send_file(int(chat_id), cyborg, reply_to=reply_to_id)
        await cyborg.delete()
