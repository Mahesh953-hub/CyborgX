...
from ..helpers.utils import _format, _cyborgutils
...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "usage": "{tr}ls <path>",
        "examples": "{tr}ls pyCyborgX",
    },
)
async def lst(event):  # sourcery no-metrics
    "To list all files and folders."
    cyborg = "".join(event.text.split(maxsplit=1)[1:])
    path = cyborg or os.getcwd()
    if not os.path.exists(path):
        await eor(
            event,
            f"there is no such directory or file with the name `{cyborg}` check again",
        )
        return
    path = Path(cyborg) if cyborg else os.getcwd()
    if os.path.isdir(path):
        ...
        for contents in sorted(lists):
            cyborgpath = os.path.join(path, contents)
            if not os.path.isdir(cyborgpath):
                size = os.stat(cyborgpath).st_size
...
async def lst(event):
    ...
    if not os.path.exists(path):
        ...
        return
    cyborgcmd = f"rm -rf {path}"
    if os.path.isdir(path):
        await _cyborgutils.runcmd(cyborgcmd)
...
async def _(event):
    ...
    try:
        await _cyborgutils.runcmd(f"mkdir {original}")
        await mone.eor(f"Successfully created the directory `{original}`")
...
async def _(event):
    ...
    await asyncio.sleep(2)
    try:
        await _cyborgutils.runcmd(f"cp -r {original} {location}")
        await mone.eor(f"Successfully copied the `{original}` to `{location}`")
    except Exception as e:
        await edit_delete(mone, str(e), parse_mode=_format.parse_pre)
...
async def _(event):
    ...
    try:
        ...
        await mone.eor(f"Successfully moved the `{original}` to `{location}`")
    except Exception as e:
        await edit_delete(mone, str(e), parse_mode=_format.parse_pre)
