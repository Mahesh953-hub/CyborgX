...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = await Cyborg(GetFullUserRequest(LASTFM_.USER_ID))
            ...
            if (
                ...
                try:
                    ...
                    await Cyborg(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await Cyborg(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await Cyborg(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await Cyborg.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await Cyborg.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
...
async def last_fm(lastFM):
    ...
    if preview is not None:
        await lastFM.eor(f"{output}", parse_mode="md", link_preview=True)
    else:
        await lastFM.eor(f"{output}", parse_mode="md")
...
async def lastbio(lfmbio):
    ...
    if arg == "on":
        ...
        if not LASTFM_.LASTFMCHECK:
            ...
            await lfmbio.eor(LFM_BIO_ENABLED)
            await sleep(4)
            await get_curr_track(lfmbio)
        else:
            await lfmbio.eor(LFM_BIO_RUNNING)
    elif arg == "off":
        LASTFM_.LASTFMCHECK = False
        LASTFM_.RUNNING = False
        await lfmbio.client(UpdateProfileRequest(about=DEFAULT_BIO))
        await lfmbio.eor(LFM_BIO_DISABLED)
    else:
        await lfmbio.eor(LFM_BIO_ERR)
...
async def lastlog(lstlog):
    ...
    if arg == "on":
        ...
        await lstlog.eor(LFM_LOG_ENABLED)
    elif arg == "off":
        LASTFM_.LastLog = False
        await lstlog.eor(LFM_LOG_DISABLED)
    else:
        await lstlog.eor(LFM_LOG_ERR)
