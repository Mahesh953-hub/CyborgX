...
from ..helpers import _cyborgtools, media_type, progress, reply_id

plugin_category = "utils"


FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "CyborgX.media.ffmpeg"
)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                cyborgevent, "**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            ...
            return await edit_delete(cyborgevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        # output should be image
        cmd, start_time = cmt
        o = await _cyborgtools.take_screen_shot(FF_MPEG_DOWN_LOAD_MEDIA_PATH, start_time)
        if o is None:
            return await edit_delete(
                cyborgevent, "**Error : **`Can't complete the process`"
            )
        ...
        await edit_delete(cyborgevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(cyborgevent, f"`Completed Process in {ms} seconds`", 3)
