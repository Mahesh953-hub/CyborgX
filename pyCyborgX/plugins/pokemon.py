...
from pyCyborgX.sql_helper.poke_sql import add_grp, get_all_grp, is_pokemon, rm_grp
from . import *
...
async def _(event):
    cyborgevent = await eor(event, "pika..")
    ...
    await cyborgevent.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)
        ...
        if int(grps.chat_id) == event.chat_id:
            try:
                ...
                try:
                    ...
                    pass
                cyborgevent = await event.client.send_message(
...
async def _(event):
    cyborgevent = await eor(event, "Fetching Auto Pokemon chats...")
    all_grp = get_all_grp()
