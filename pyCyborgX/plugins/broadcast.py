...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @RemainsAlways just a testmessage",
    },
)
async def cyborgbroadcast_add(event):
...
async def cyborgbroadcast_add(event):
    ...
    cyborginput_str = event.pattern_match.group(1)
    if not cyborginput_str:
        return await edit_delete(
            event,
            "In which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = cyborginput_str.lower()
...
async def cyborgbroadcast_list(event):
    ...
    await eor(cyborgevent, finaloutput)
...
async def cyborgbroadcast_send(event):
    ...
    await edit_delete(cyborgevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def cyborgbroadcast_remove(event):
...
async def cyborgbroadcast_delete(event):
    "To delete a category completely."
    cyborginput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(cyborginput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {cyborginput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(cyborginput_str)
        await eor(
            event,
            f"Successfully deleted the category {cyborginput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
