...
async def _(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(URL) as request:
            ...
            cyborgevent = await eor(event, "`fetching github info ...`")
...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "description": "It uploads the given file to your github repo in **pyCyborgX/plugins** folder\
...
async def git_commit(file_name, mone):
    ...
    if create_file:
        file_name = f"pyCyborgX/plugins/{file_name}"
        LOGS.info(file_name)
        try:
            repo.create_file(
            ...
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your PLUGINS](https://github.com/{ccess}/tree/master/pyCyborgX/plugins/)"
            )
