...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using CyborgX.",
        "mimeType": mimeType,
...
async def share(service, event, url):
    """get shareable link"""
    await event.eor("`Loading GDrive Share...`")
    ...
    try:
        ...
        return
    await event.eor(f"**Shareable Links**\n\n{result}")
        ...
        try:
            if not complete:
                if not file.error_message:
                    ...
                    await event.eor(f"Error : `{str(file.error_message)}`")
                    return
            else:
                await event.eor(
                    f"**Name : **`{file.name}`\n"
            ...
            if " not found" in str(e) or "'file'" in str(e):
                await event.eor("Download Canceled :\n`{}`".format(file.name))
...
@Cyborg.on_cmd(
    ...
    info={
        ...
        "description": "G-Drive File Downloader Plugin For CyborgX. only gdrive files are supported now",
        "flags": {
...
async def g_download(event):
    ...
    file_name, cyborgprocess = await gdrive_download(
        event, cyborgevent, service, drive_link
    )
    if cyborgprocess is not None:
        return await edit_delete(cyborgevent, file_name)
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, cyborgevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
async def gshare(event):
    ...
    cyborgevent = await eor(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, cyborgevent, input_str)
