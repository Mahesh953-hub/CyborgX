...
from . import deEmojify, eor, Cyborg
...
async def carbon_api(event):
    ...
    cyborg = await eor(event, "`Carbonizing...\n25%`")
    ...
    await cyborg.eor("`Processing..\n75%`")

    await asyncio.sleep(2)
    await cyborg.eor("`Done Dana Done...\n100%`")
    file = "./carbon.png"
    await cyborg.eor("`Uploading..`")
...
async def carbon_api(event):
    ...
    pcode = deEmojify(pcode)
    code = quote_plus(pcode)
    await cyborg.eor("`Meking Carbon...`\n`25%`")
    url = CARBON.format(code=code, lang=CARBONLANG)
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.binary_location = Config.CHROME_BIN
...
async def carbon_api(event):
    ...
    await cyborg.eor("🔳🔳🔳🔳🔳")
    file = "./carbon.png"
    await cyborg.eor("☣️Karbon1 Completed, Uploading Karbon☣️")
    await event.client.send_file(
        event.chat_id,
        file,
        force_document=True,
...
async def carbon_api(event):
    ...
    await cyborg.eor("🔘🔘🔘🔘🔘")
    file = "./carbon.png"
    await cyborg.eor("☣️Karbon2 Completed, Uploading Karbon☣️")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def carbon_api(event):
    ...
    await cyborg.eor("🔵🔵🔵🔵🔵")
    file = "./carbon.png"
    await cyborg.eor("☣️Karbon3 Completed, Uploading Karbon⬆️")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def carbon_api(event):
    ...
    await cyborg.eor("🌝🌝🌝🌝🌝")
    file = "./carbon.png"
    await cyborg.eor("✅Karbon4 Completed, Uploading Karbon✅")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def carbon_api(event):
    ...
    await cyborg.eor("⬛⬛⬛⬛⬛")
    file = "./carbon.png"
    await cyborg.eor("✅RGB Karbon Completed, Uploading Karbon✅")
    await event.client.send_file(
        event.chat_id,
        file,
